...
import { UnlockSVG } from '../../icons/Unlock';
                ...
                if (fromCertificateBuilder) {
                    ...
                    RedirectToCourse(item);
                        ...
                        alignItems: 'center',
                            ...
                            }, children: [fromWizard && (_jsx(Checkbox, { onChange: function () { return selectItem && selectItem(item.id); }, checked: selectedItem && selectedItem.includes(item.id) })), _jsx(CourseStatusContainer, { "$status": item.state === 'published', children: item.finished === 3 ? getCourseState(item.state || '') : t('courses_layout.draft') })] }), actions && actions.length > 0 && (_jsx("div", { className: 'action_block', children: _jsx(GridActionMenu, { actionConfig: actions, row: item }) }))] }) }), _jsxs(GridItem, { "$isFromUser": Boolean(groupId || user), children: [_jsx(GridItemImage, { children: _jsx("img", { src: (item.avatar && item.avatar.link) ||
                                'https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcSHw6OeiQMdYX_iXu8otmzMnlbD07xEWr87EMnJ5r4X9vYC-kNr&usqp=CAU' }) }), _jsxs(GridItemContent, { children: [_jsxs(GridItemWrapper, { style: { justifyContent: 'space-between' }, children: [_jsx(ItemBlock, { children: item.level && (_jsxs(_Fragment, { children: [t('course_details.level'), ": ", getCourseLevel(item.level)] })) }), _jsxs(ItemBlock, { style: { color: 'green' }, children: [item.currency, item.price
                                                ...
                                                : t('courses_layout.free')] })] }), _jsx(GridItemContentHeader, { children: _jsx(GridItemTitle, { style: { marginTop: '8px' }, children: _jsx(TooltipHeight, { text: item.name, lines: 3 }) }) }), _jsx(StyledItemWrapper, {})] })] }), _jsx(StyledCardBottomContainer, { style: { textTransform: 'capitalize' }, children: _jsxs(_Fragment, { children: [_jsxs(GridItemLessons, { checked: fromWizard && selectedItem && selectedItem.includes(item.id) ? true : false, children: [_jsxs(StyledCourseCardContent, { children: [item.numberOfModules, " ", t('general.modules')] }), _jsxs(StyledCourseCardContent, { children: [item.numberOfLessons, " ", t('general.lessons')] }), _jsx(StyledCourseCardContent, { children: formatDuration(item.duration || 0) })] }), actionHandler && !item.isBought && (_jsxs(StyledActionsContainer, { children: [_jsxs(StyledActionItem, { color: '#20a8c6', onClick: function (e) {
